...
export const StaggeredMenu = ({
  ...
  const [textLines, setTextLines] = useState([
    <RiMenu3Fill key="menu-icon" size="1.2em" />,
    <IoCloseSharp key="close-icon" size="1.2em" />,
  ]);
  ...
  const animateText = useCallback((opening) => {
    ...
    for (let i = 0; i < cycles; i++) {
      lastIsMenu = !lastIsMenu;
      seq.push(
        lastIsMenu
          ? <RiMenu3Fill key={`menu-${i}`} size="1.2em" />
          : <IoCloseSharp key={`close-${i}`} size="1.2em" />
      );
    }
    seq.push(targetIcon);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <header
          className="staggered-menu-header absolute top-0 w-full flex justify-end items-center mr-[5em] bg-transparent pointer-events-none z-20"
        ...
        >
          ...
          <button
            ref={toggleBtnRef}
            className="sm-toggle relative bottom-[8px] inline-flex items-center gap-[0.3rem] bg-transparent border-0 cursor-pointer text-[#e9e9ef] font-medium leading-none overflow-visible pointer-events-auto text-right"
        ...
        >
          <div className="sm-panel-inner flex-1 flex flex-col gap-5">
            ...
            >
              {items && items.length ? (
                items.map((it, idx) => (
                  ...
                  >
                    <a
                      ...
                      data-index={idx + 1}
                    >
                      <span className="sm-panel-itemLabel inline-block [transform-origin:50%_100%] will-change-transform  text-[25px] mt-5">
                        {it.label}
                      </span>
                    </a>
                  </li>
                ...
                <li
                  ...
                  aria-hidden="true"
                >
                  <span className="sm-panel-item relative text-black font-semibold  cursor-pointer leading-none tracking-[-2px] uppercase transition-[background,color] duration-150 ease-linear inline-block no-underline pr-[1.4em]">
            ...
            {displaySocials && socialItems && socialItems.length > 0 && (
              <div
                className="sm-socials flex flex-col gap-3"
...
.sm-scope .sm-socials { margin-top: 12.5rem; padding-top: 0.5rem; display: flex; flex-direction: column; gap: 0.75rem; }
.sm-scope .sm-socials-title { margin: 0; font-size: 1rem; font-weight: 500; color: var(--sm-accent, #ff0000); }
.sm-scope .sm-socials-list { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: row; align-items: center; gap: 1rem; flex-wrap: wrap; }
.sm-scope .sm-socials-list .sm-socials-link { opacity: 1; transition: opacity 0.3s ease; }
@media (max-height: 700px) { 
  .sm-scope .staggered-menu-panel { padding: 4em 2em 1.5em 2em; } 
  .sm-scope .sm-panel-inner { gap: 0.75rem; } 
  .sm-scope .sm-socials { padding-top: 0.5rem; } 
}
